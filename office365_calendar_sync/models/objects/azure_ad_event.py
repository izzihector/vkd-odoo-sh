...
class AzureADEvent:
    ...
    def get_azure_change_template(change, original):
        ...
        if 'allday' in change:
            ...
            if 'Start' not in r:
                r['Start'] = {
                    'DateTime': fields.Datetime.from_string(original.start).replace(hour=0, minute=0, second=0, microsecond=0).strftime(DATETIME_FORMAT) if r['IsAllDay'] else fields.Datetime.from_string(original.start).strftime(DATETIME_FORMAT),
                    'TimeZone': 'UTC',
                }

            if 'End' not in r:
                r['End'] = {
                    'DateTime': (fields.Datetime.from_string(original.stop).replace(hour=0, minute=0, second=0, microsecond=0) + timedelta(seconds=1)).strftime(DATETIME_FORMAT) if r['IsAllDay'] else (fields.Datetime.from_string(original.stop) + timedelta(days=1)).strftime(DATETIME_FORMAT),
                    'TimeZone': 'UTC',
                }
