...
class CalendarEvent(models.Model):
    ...
    def create_link(self, partner_id=None):
        ...
        ad_event = AzureADEvent(
            ...
            start_date=fields.Datetime.from_string(self.start) if not self.allday else fields.Datetime.from_string(self.start[:10]),
            end_date=fields.Datetime.from_string(self.stop) if not self.allday else fields.Datetime.from_string(self.stop[:10]) + timedelta(days=1),
            all_day=self.allday,
