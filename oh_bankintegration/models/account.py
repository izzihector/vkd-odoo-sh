...
class AccountJournal(models.Model):
    ...
    bank_statements_source = fields.Selection(
        selection_add=[("bankintegration_import", "Auto import from bankintegration")])
        ...
        try:
            ...
            if bank_stmt_res:
                ...
                if bank_stmt_line_res:
                    last_import_date = parse(bank_stmt_line_res.date)
    ...
    def bankintegration_statement_auto(self):
        journal_ids = self.search([('bank_statements_source', '=', 'bankintegration_import')])
        for journal in journal_ids:
            ...
            auth_header, request_id = account_invoice_model.get_bank_statement_token(
                request_obj.id, journal_id)
            last_import_date, last_import_balance = self.get_last_import_date(journal_id)
            stmts_data = account_invoice_model.get_bank_statements(
                auth_header, request_id, last_import_date, last_import_balance)
            if stmts_data:
                bank_statement_import_model.bankintegration_import_statements(
