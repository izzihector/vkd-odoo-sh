...
class AzureAdUser(models.Model):
    ...
    def set_token(self, grant_type, code_type, code):
        ...
        try:
            ...
            _logger.debug('GetToken - Authentication failure for user %s: %s' % (self.id, str(e)))
        ...
        try:
            ...
            _logger.debug('JWT - Authentication failure for user %s: %s --- %s' % (self.id, str(e), str(response)))
        ...
        try:
            ...
            _logger.debug(jwt)
            _logger.debug('Write - Authentication failure for user %s: %s' % (self.id, str(e)))
