...
class OocCreationModel(models.Model):
    ...
    subject_field_name = fields.Char(related='subject_field_id.name', store=True)
    ...
    message_field_name = fields.Char(related='message_field_id.name', store=True)
    message_field_type = fields.Selection(related='message_field_id.ttype')

    from_email_field_id = fields.Many2one('ir.model.fields', string='From Email field')
    from_email_field_name = fields.Char(related='from_email_field_id.name', store=True)

    from_name_field_id = fields.Many2one('ir.model.fields', string='From Name field')
    from_name_field_name = fields.Char(related='from_name_field_id.name', store=True)
